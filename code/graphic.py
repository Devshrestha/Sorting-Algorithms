...
def bars(array,change):
    ...
    for x in range(n):
        if x in change:
            pygame.draw.rect(screen,red,[(w_gap*(x+1))+w*x,display_height-arry[x]-shift,w,arry[x]])
        else:
            pygame.draw.rect(screen,white,[(w_gap*(x+1))+w*x,display_height-arry[x]-shift,w,arry[x]])
